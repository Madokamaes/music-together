...
| 功能               | 说明                                                                                                                                  |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| 房间系统           | 创建/加入永久房间，房主主动解散前不会消失；成员离线后保留在房间名单中，可选密码保护，也可隐藏为仅完整房间号/邀请链接可加入            |
| 多音源搜索         | 网易云、QQ音乐、酷狗                                                                                                                  |
| 同步播放           | 房间内播放进度实时同步                                                                                                                |
| 实时聊天           | 房间内文字聊天，消息持久化并保存发送者昵称/头像快照                                                                                   |
...
| 用户资料           | Cookie 身份对应持久化昵称、头像和一次性设置的账号密码；头像上传后统一压缩为 WebP，无上传头像时前端生成默认渐变头像                    |
| 听歌统计           | 每次成功开始播放歌曲时记录房间、歌曲和当时在线用户快照，供后续年度总结使用                                                            |
| 幻想音乐杯（外部） | 首页固定链接到独立服务 `http://47.94.44.206:3002/`；代码、API、媒体和部署位于 `Madokamaes/music-cup` 仓库，不属于本应用运行时 |

### 技术栈

- **前端**: React 19 + Vite 7 + TypeScript 5.9 + Tailwind CSS v4 + shadcn/ui + Zustand
- **后端**: Node.js + Express 4 + Socket.IO 4 + @meting/core + SQLite (`better-sqlite3`)
...
| 9. 部署方案     | [deployment.md](architecture/deployment.md)                   | Docker、GitHub Actions SSH 部署、持久化、CORS、1Panel                              |
